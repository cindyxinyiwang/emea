...
class AdamW(Optimizer):
    ...
    def __init__(
        ...
        self.optimized_params_copy = []

    def params(self):
        if self.optimized_params is not None: return self.optimized_params
        self.optimized_params = []
        for group in self.param_groups:
            for p in group["params"]:
                if p.requires_grad:
                    self.optimized_params.append(p)
        return self.optimized_params
    
    def add_meta_step(self, grad):
        lr = self.param_groups[0]["lr"]
        for p, g in zip(self.optimized_params, grad):
            p_copy = p.clone().detach()
            self.optimized_params_copy.append(p_copy)
            p.data.add_(-lr * g)

    def reset_param(self):
        for p, p_copy in zip(self.optimized_params, self.optimized_params_copy):
            p.data.copy_(p_copy)
        self.optimized_params_copy = []
