...
class BertSelfOutputAdaptersMixin:
    ...
    def adapters_forward(self, hidden_states, input_tensor, adapter_names=None, adapter_weights=None):
...
class BertOutputAdaptersMixin:
    ...
    def adapter_stack_layer(self, hidden_states, input_tensor, adapter_stack, adapter_weights=None):
        """
        One layer of stacked adapters. This either passes through a single adapter and prepares the data to be passed
        into a subsequent adapter, or the next transformer layer
        OR
        ...
        if len(adapter_stack) == 1:
            ...
            if adapter_weights is None:
                ...
                return self.adapter_parameter_fusion(hidden_states, adapter_stack, residual, query, adapter_weights=adapter_weights)

    def adapter_parameter_fusion(self, hidden_states, adapter_stack, residual, query, adapter_weights=None):
        adapter_layer = self.get_adapter_layer(adapter_stack[0])
        ...
        if adapter_weights is not None:
            ...
            for i, adapter_name in enumerate(adapter_stack):
                adapter_layer = self.get_adapter_layer(adapter_name)
                output, _, _ = adapter_layer(hidden_states, residual_input=residual)
                if outs is None:
            ...
            #        #p1 = p1 + adapter_weights[i]*p2
            #hidden_states, _, _ = fused_adapter_layer(hidden_states, residual_input=residual)
        else:
            for adapter_name in adapter_stack:
                adapter_layer = self.get_adapter_layer(adapter_name)
                for p1, p2 in zip(fused_adapter_layer.parameters(), adapter_layer.parameters()):
