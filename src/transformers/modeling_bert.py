...
class BertSelfOutput(BertSelfOutputAdaptersMixin, nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, adapter_names=None, adapter_weights=None):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.adapters_forward(hidden_states, input_tensor, adapter_names=adapter_names, adapter_weights=adapter_weights)
...
class BertOutput(BertOutputAdaptersMixin, nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, adapter_names=None, adapter_weights=None):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.adapters_forward(hidden_states, input_tensor, adapter_names, adapter_weights)
        return hidden_states
...
class BertLayer(BertLayerAdaptersMixin, nn.Module):
    ...
    def forward(
        ...
        #layer_output = self.feed_forward_chunk(attention_output, adapter_names, adapter_weights)
        outputs = (layer_output,) + outputs
        return outputs

    def feed_forward_chunk(self, attention_output, adapter_names=None, adapter_weights=None):
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output, adapter_names=adapter_names, adapter_weights=adapter_weights)
        return layer_output
            ...
            if getattr(self.config, "gradient_checkpointing", False):
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(*inputs, output_attentions, adapter_names=adapter_names, adapter_weights=adapter_weights)
                ...
                if adapter_weights is not None and type(adapter_weights[0]) != float:
