...
class InvertibleAdaptersMixin:
    ...
    def get_invertible_lang_adapter(self, adapter_names):
        if self.fused_adapter_layer is not None:
            return self.fused_adapter_layer
    ...
    def invertible_adapters_forward(self, hidden_states, adapter_names=None, rev=False, adapter_weights=None):
        # TODO: Currently no fusion over invertible adapters, takes only very first language adapter position
        if adapter_names is not None and len(adapter_names) > 0:
            adapter_names = parse_adapter_names(adapter_names)
            #print(adapter_names, self.invertible_lang_adapters)
            if len(adapter_names[0]) > 1 and adapter_weights is not None:
                hidden_states = self.parameter_fusion_invertible_adapters_forward(hidden_states, adapter_names[0], rev=rev, adapter_weights=adapter_weights)
            else:
                if False:
                #if len(adapter_names[0]) > 1:
                    hidden_states = self.parameter_fusion_invertible_adapters_forward(hidden_states, adapter_names[0], rev=rev, adapter_weights=adapter_weights)
                else:
    ...
    def parameter_fusion_invertible_adapters_forward(self, hidden_states, adapter_stack, rev=False, adapter_weights=None):
        adapter_layer = self.invertible_lang_adapters[adapter_stack[0]]
        #fused_adapter_layer = copy.deepcopy(adapter_layer)
        #for n, p in fused_adapter_layer.named_parameters():
        #    p.fill_(0)
        if adapter_weights is not None:
            outs = None
            for i, adapter_name in enumerate(adapter_stack):
                adapter_layer = self.invertible_lang_adapters[adapter_name]
                output = adapter_layer(hidden_states, rev=rev)
                if outs is None:
                    outs = output*adapter_weights[i]
                else:
                    outs = outs + output*adapter_weights[i]
            hidden_states = outs
            #for i, adapter_name in enumerate(adapter_stack):
            #    adapter_layer = self.invertible_lang_adapters[adapter_name]
            #    for p1, p2 in zip(fused_adapter_layer.parameters(), adapter_layer.parameters()):
            #        p1.copy_(p1 + adapter_weights[i]*p2)
            #hidden_states = fused_adapter_layer(hidden_states, rev=rev)
        else:
            for adapter_name in adapter_stack:
                adapter_layer = self.invertible_lang_adapters[adapter_name]
                for p1, p2 in zip(fused_adapter_layer.parameters(), adapter_layer.parameters()):
                    p1.copy_(0.5*p1 + 0.5*p2)
            hidden_states = fused_adapter_layer(hidden_states, rev=rev)
        #self.fused_adapter_layer = fused_adapter_layer
        return hidden_states
...
class ModelAdaptersMixin(ABC):
    ...
    def set_active_adapters(self, adapter_names: list):
        ...
        if len(adapter_names) == 0:
            self.base_model._active_adapter_names = None
            return
    ...
    def freeze_model(self, freeze=True):
        ...
        for n, param in self.base_model.named_parameters():
