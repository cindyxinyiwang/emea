...
def qa_results(filename, do_final=1):
    ...
    print(
        "xquad ave f1 over random seeds:",
        combined_df.xs('f1').mean(axis=1).mean())
    print(
        "xquad ave exact f1 over random seeds:",
        combined_df.xs('exact f1').mean(axis=1).mean())
    ...
    for seed in seeds:
        results = {}
        exact_results = {}
        c_f = filename + "_s{}/predictions/mlqa/mlqa_test_results_{}.txt".format(
            seed, do_final)
        if os.path.exists(c_f):
            with open(c_f, 'r') as f:
                for line in f:
    ...
    print(combined_df.mean(axis=1))
    print(
        "mlqa ave f1 over random seeds:",
        combined_df.xs('f1').mean(axis=1).mean())
    print(
        "mlqa ave exact f1 over random seeds:",
        combined_df.xs('exact f1').mean(axis=1).mean())
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", type=str, default="tag", help="[tag|classify|qa]")
